    ...
    public sealed partial class BlankPagePasport : Page
    {
      ...
      public  ObservableCollection<ClassPasport> classPasports = new ObservableCollection<ClassPasport>();
                    ...
                    foreach (int kl in nameKl)
                    {
                        foreach (string t in mecdata)
                        {
                            ...
                            if (d.Count() > 0)
                            {
                                ...
                                foreach (var ff in await storageFolder.GetFoldersAsync())
                                {

                                    if (ff.DisplayName == "Assets")
                                    {
                                        

                                        StorageFile storageFile = await ff.GetFileAsync("ШаблонТекстПаспорт.rtf");
        ...
        //    var dH = radChart.HorizontalAxis.TitleTemplate;
         ...
         //   var LS = radChart.VerticalAxis.LabelStyle;
         //   var LT = radChart.VerticalAxis.MajorTickStyle;
            //  radChart.VerticalAxis = new Telerik.UI.Xaml.Controls.Chart.LinearAxis() { Maximum =1, LineStyle=ST, MajorTickStyle = LT, LabelStyle = LS, Minimum = 0, Foreground = new SolidColorBrush(Colors.Black), TitleTemplate=df };
            //  radChart.HorizontalAxis = new DateTimeCategoricalAxis() { MajorTickInterval = d, MajorTickStyle = LT, LabelStyle = LS, LineStyle =ST, DateTimeComponent = Telerik.Charting.DateTimeComponent.Date, LabelFormat = "{0,0:dd.MM.yy}", Foreground = new SolidColorBrush(Colors.Black), TitleTemplate=dH };
            //  radChart.VerticalAxis.Title = "N(Sd)/Ns";
            //  radChart.HorizontalAxis.Title = "Дата";
            //  radChart.Palette = CustomPalettes.CustomDark;
            _MainViewModel = new MainViewModel(SigM);
            oxi.Model = _MainViewModel.MyModel;
        ...
        public async Task<StorageFile> creatPicTempSobN(List<ClassTemp> SigM)
        {
         ...
         //   if (SigM.Count() > 4)
          ...
          //  var LS = radChart.VerticalAxis.LabelStyle;
          //  var LT = radChart.VerticalAxis.MajorTickStyle;
          //  radChart.VerticalAxis = new Telerik.UI.Xaml.Controls.Chart.LinearAxis() { Maximum = 1.0, LineStyle=ST, Minimum = 0, MajorTickStyle = LT, LabelStyle = LS, Foreground = new SolidColorBrush(Colors.Black), TitleTemplate=df };
          //  radChart.HorizontalAxis = new DateTimeCategoricalAxis() { MajorTickInterval =d, LineStyle=ST, MajorTickStyle = LT, LabelStyle = LS, DateTimeComponent = Telerik.Charting.DateTimeComponent.Date, LabelFormat = "{0,0:dd.MM.yy}", Foreground = new SolidColorBrush(Colors.Black), TitleTemplate=dH };
          //  radChart.VerticalAxis.Title = "N нейтронов детектором/N событий ";
          //  radChart.HorizontalAxis.Title = "Дата";
          //  radChart.Palette = CustomPalettes.CustomDark;
